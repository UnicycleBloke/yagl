...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void SpriteLayout::read(std::istream& is)
{
    ...
    // Advanced building sprite, or 0x01 for a single Extended building sprite.
    uint8_t num_sprites = read_uint8(is);

    m_format = Format::Basic;
    if ((num_sprites & 0x3F) > 0) m_format = Format::Extended;
    if (num_sprites & 0x40)       m_format = Format::Advanced;
    num_sprites &= 0x3F;
    ...
    if (m_format == Format::Basic)
    {
        ...
        sprite.zofs   = 0; // No Z for Basic layout
        sprite.new_bb = true;
        sprite.xext   = read_uint8(is);
        sprite.yext   = read_uint8(is);
        sprite.zext   = read_uint8(is);

        // We only have one sprite, but use the same data structure 
        // as for Extended and Advanced for simplicity.
...
void SpriteLayout::write(std::ostream& os) const
{
    // Don't we already know m_format from the parsing?
    ...
    if (!extended_format)
    {
        const Sprite& sprite = m_building_sprites[0];
        write_uint32(os, sprite.sprite);
        write_uint8(os, sprite.xofs);
        write_uint8(os, sprite.yofs);
        //write_uint8(os, sprite.zofs); // Not present for Basic layout.
    ...
    else
    {
        for (uint8_t i = 0; i < num_sprites; ++i)
        {
            const Sprite& sprite = m_building_sprites[i];
            write_uint32(os, sprite.sprite);
            if (m_format == Format::Advanced)
            {
...
void SpriteLayout::print(std::ostream& os, uint16_t indent) const
{
    // This differs slightly from the layout used for Houses etc. Just in the use 
    // of whitespace.
    os << '\n' << pad(indent) << "{\n";
...
void SpriteLayout::parse(TokenStream& is)
{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void SpriteLayout::Registers::read(std::istream& is,  bool is_parent)
{
...
void SpriteLayout::Registers::write(std::ostream& os, bool is_parent) const
{
...
void SpriteLayout::Registers::print(std::ostream& os, bool is_parent, uint16_t indent) const
{
    if ((flags & 0xFF) > 0)
    {
...
void SpriteLayout::Registers::parse(TokenStream& is,  bool is_parent)
{
    is.match(TokenType::OpenBrace);
    while (is.peek().type != TokenType::CloseBrace)
    {
...
void SpriteLayout::parse_ground_sprite(TokenStream& is)
{
...
void SpriteLayout::parse_building_sprite(TokenStream& is)
{
    Sprite sprite;
    sprite.new_bb = true;
    is.match(TokenType::OpenAngle);
...
void SpriteLayout::parse_child_sprite(TokenStream& is)
{
    Sprite sprite;
    sprite.new_bb = false;
    is.match(TokenType::OpenAngle);
